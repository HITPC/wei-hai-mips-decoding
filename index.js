...
explain.addEventListener('click', ()=>{
  alert("使用须知：本网页并没有实现全部的指令，有部分指令如lbu，sw，sh，sb，lw，lhu等没有写，别的可能也有缺的。请一定要确保转换前输入的指令格式是对的，指令格式，必须要英文逗号并且逗号后面打一个空格，并且末尾不需要加任何东西！注意，如果指令是对的但是格式错误/部分使用方式错误情况，结果会出现undefined，但是如果是使用方式错了，本页面没有细致检查使用方式正确与否的功能，所以会正常译码，此时结果会看着没问题（这个就会出大问题了）");
});
